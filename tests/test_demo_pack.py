...
def _init_repo(tmp_path: Path) -> tuple[str, str]:
    ...
    subprocess.run(['git', 'add', 'src/main.py'], cwd=tmp_path, check=True, capture_output=True, text=True)
    subprocess.run(
...
def test_write_demo_pack_creates_markdown(tmp_path):
    ...
    limitations_path.write_text('- Requires manual refresh for nightly jobs.', encoding='utf-8')
    next_steps_path = sprint_root / 'next_steps.md'
    next_steps_path.write_text('- Automate dashboard refresh schedule.', encoding='utf-8')
