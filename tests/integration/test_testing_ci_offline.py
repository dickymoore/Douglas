...
from douglas.steps.testing import TestConfig, TestingStep
...
def test_testing_step_deterministic(tmp_path):
    config = TestConfig(
        ...
        test_count=8,
        failure_rate=0.25,
        coverage_range=(0.55, 0.72),
    )
    step = TestingStep(tmp_path, config=config)
    result_first = step.run()

    # Running with the same configuration should be deterministic.
    step_repeat = TestingStep(tmp_path, config=config)
    ...
    # Coverage stays within the requested range (converted to percentages).
    ...
    replay = TestingStep(tmp_path, config=config).run(replay=True)
