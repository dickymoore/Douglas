...
class TestConfig:
    ...
    coverage_range: tuple[float, float] = (0.65, 0.9)
...
class TestingStep:
    ...
    def __init__(
        ...
        self.config = config or TestConfig(seed=0)
    ...
    def _generate_coverage(self, rng: random.Random) -> Coverage:
        ...
        if high <= 1.0:
            percent = rng.uniform(low, high) * 100
        else:
...
__all__ = ["TestConfig", "TestingStep"]

TestConfig.__test__ = False  # type: ignore[attr-defined]
TestingStep.__test__ = False  # type: ignore[attr-defined]
